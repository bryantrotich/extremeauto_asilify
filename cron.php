 ...
 * 
 */
$subscriptions = Asilify::subscriptions();
if (!empty($subscriptions)) {
	foreach ($subscriptions as $key => $subscription) {
		$company = Database::table("companies")->where("subscription_id", $subscription->id)->first();
		if (!empty($company)) {
...
/**
 * Mark expired free trials
 * 
 */
$companies = Database::table("companies")->where("subscription_status", "Active")->where("subscription_end","<=", $today)->get();
if (!empty($companies)) {
