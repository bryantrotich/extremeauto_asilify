...
class Tasks {
    ...
    public function daily() {
        ...
        $title = 'Tasks on '.date("d/m/Y", strtotime($date));
        
        $tasks = Database::table('tasks')->where('company', $user->company)->where('due_date',">=", $date)->orderBy("id", false)->get();
        ...
        return view("tasks-daily", compact("user", "title", "tasks", "date"));
    ...
    public function create() {
        ...
        if (empty($_POST["members"])) {
            return response()->json(responder("error", "Hmmm!", "Select atleast one staff member."));
        }
        ...
        foreach($_POST["members"] as $memberid){
            ...
            if ($user->parent->sms_tasks == "Enabled") {
                $member = Database::table('users')->where('company', $user->company)->where('id', $memberid)->first();
                if (!empty($member->phonenumber)) {
                    $message = "New Task #".$taskid."!\nTitle: ".input('title')."!\nVehicle: ".$project->registration_number."\nCost: ".money(input('cost'), $user->parent->currency)."\nDue Date: ".date("F j, Y", strtotime(input('due_date'))).$dueTime."\nDescription: ".nl2br(input('description'))."\n\n ".$user->parent->name;
    ...
    public function complete() {
        ...
        Database::table('tasks')->where('id', input('taskid'))->where('company', $user->company)->update($data);
        return response()->json(responder("success", "Alright!", "Task successfully Completed.", "reload()"));
