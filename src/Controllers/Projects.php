...
class Projects {
    ...
    /**
     * Render completed projects page
     * 
     * @return \Pecee\Http\Response
     */
    ...
    public function checkout() {
        ...
        $pendingtasks = Database::table('tasks')->where('company', $user->company)->where('project', $project->id)->where('status', "In Progress")->orderBy("id", false)->get();
        if (!empty($pendingtasks) && $user->parent->restricted_checkout == "Enabled") {
            $allowCheckOut = false;
            $message = "Some pending tasks on this project. Mark them as completed and try again.";
        }
        $undelivereds = Database::table('expenses')->where('company', $user->company)->where('project', $project->id)->where('status',"!=","Delivered")->orderBy("id", false)->get();
        if (!empty($undelivereds) && $user->parent->restricted_checkout == "Enabled") {
            $allowCheckOut = false;
            $message = "Some undelivered parts/service on this project. Mark them as delivered and try again.";
        }
    ...
    public function booking() {
        ...
        $makes = Database::table('makes')->where('status', "Enabled")->orderBy("name", true)->get();
    ...
    public function complete() {
        ...
        $data = array(
            "status" => escape("Completed")
        );
        
        Database::table('projects')->where('id', input('projectid'))->where('company', $user->company)->update($data);
        return response()->json(responder("success", "Alright!", "Project successfully marked as complete.", "reload()"));
