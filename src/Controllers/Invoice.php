...
class Invoice {
    ...
    public function create() {
        ...
        foreach ($_POST["item"] as $key => $item) {
            ...
            $line["total"] = $this->linetotal((object) $line);
        ...
        if (!empty(input("vat")) && input("vat") > 0) {
            $update["tax"] = input("vat");
            $update["tax_amount"] = round(((input("vat") / 100) * $update["subtotal"]), 2);
            $update["total"] = $update["tax_amount"] + $update["subtotal"];
        }else{
    ...
    public function update() {
        ...
        if (!empty(input("vat")) && input("vat") > 0) {
            $data["tax"] = input("vat");
            $data["tax_amount"] = round(((input("vat") / 100) * $data["subtotal"]), 2);
            $data["total"] = $data["tax_amount"] + $data["subtotal"];
        }else{
            $data["tax"] = $data["tax_amount"] = 0.00;
    ...
    public function generate($user, $invoice, $invoiceitems, $project, $client, $save = false) {
        ...
        $pdf->SetFont('','',10);
        $pdf->SetTextColor(128, 148, 174);
        $pdf->MultiCell(60, 20, "Tax ".$invoice->tax."% ", null, "R");
        $pdf->SetXY($xPos, $yPos);
