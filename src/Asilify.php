...
class Asilify {
    ...
    /**
     * Check if any job card has been created
     * 
     * @return mixed
     */
    public static function jobcard($projectid) {
        
        $jobcards = Database::table('jobcards')->where('project', $projectid)->get();
        
        if(!empty($jobcards)){
            return true;
        }else{
            return response()->json(responder("warning", "Alright!", "A Job card has to be created for this project before you can perfom this action."));
        }
